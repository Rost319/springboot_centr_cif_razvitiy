...
public class NewsRESTControllerTest {
    ...
    public void updateTest() throws Exception {
        ...
        String inputJson = writeValue(news);
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.put(REST_URL + 100001)
                .contentType(MediaType.APPLICATION_JSON_VALUE).content(inputJson))
                .andExpect(status().isOk())
                .andReturn();
    ...
    public void deleteTest() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.delete(REST_URL + 100000))
                .andExpect(status().isOk())
                .andReturn();

        assertEquals(getContent(mvcResult), "News with ID = 100000 was deleted");
